extern void* __translate_function(void*);
void* in6addr_any__addr(void);
void* in6addr_loopback__addr(void);
void* fast_stack__addr(void);
void* sockfd__addr(void);
void* servaddr__addr(void);
void* clientaddr__addr(void);
void* clientlen__addr(void);
extern int mpns_likely(int condition, const char* name, int id, void* old_rsp);
extern int mpns_unlikely(int condition, const char* name, int id, void* old_rsp); 
static inline int mpns_unknown(int condition, const char* name, int id, void* fn){return condition;} 
...
# 1 "/home/kmohr/rot13_echo/instrumentation/lakeplacid.h" 1
# 12 "/home/kmohr/rot13_echo/instrumentation/lakeplacid.h"
...
stack alloc_stack(uint64_t size);
void free_stack(stack s);

ring_buffer* create_ring_buffer(int size );
# 7 "/home/kmohr/rot13_echo/server.c" 2
...
int handle_request(void) {
    ...
    int n = recvfrom((*(int *)(sockfd__addr())), buffer, sizeof(buffer), 0, (struct sockaddr*) &(*(struct sockaddr_in *)(clientaddr__addr())), &(*(int *)(clientlen__addr())));
    if (mpns_unlikely((n < 0) && 1, "handle_request", 0, fast_stack.rsp)) {
        return -1;
    }

    if (mpns_unlikely((buffer[0] == '?') && 1, "handle_request", 1, fast_stack.rsp)) {
        for (int i = 0; i < n; i++) {
            if (mpns_unknown((buffer[i] >= 'A' && buffer[i] <= 'Z') && 1, "handle_request", 2, fast_stack.rsp)) {
                buffer[i] = 'A' + (buffer[i] - 'A' + 13) % 26;
            } else if (mpns_unknown((buffer[i] >= 'a' && buffer[i] <= 'z') && 1, "handle_request", 3, fast_stack.rsp)) {
                buffer[i] = 'a' + (buffer[i] - 'a' + 13) % 26;
            }
        }
    }

    n = sendto((*(int *)(sockfd__addr())), buffer, n, 0, (struct sockaddr*) &(*(struct sockaddr_in *)(clientaddr__addr())), (*(int *)(clientlen__addr())));
    if (mpns_unlikely((n < 0) && 1, "handle_request", 4, fast_stack.rsp)) {
        return -1;
    }

    return 0;
}void* __attribute__((weak)) handle_request__addr(void) { return (void*)&handle_request;}
